...
if os.path.exists('final-db-output.txt'):
  os.remove("final-db-output.txt")
...
with open("db-result.txt", "r") as read_handle:
  with open("final-db-output.txt", "a") as write_handle:
    for line in read_handle:
      ...
      if (from_llvm):
        ...
        if len(souper_dict) != len(llvm_dict):
          ...
          write_handle.write("Error: mismatched number of variables\n")
          souper_dict.clear()
          llvm_dict.clear()
        else:
          ...
          for (k,v), (k2,v2) in zip(od_souper.items(),od_llvm.items()):
            ...
            if len(souper_bits[1]) != len(llvm_bits[1]):
              write_handle.write("Error: mismatched length\n")
            else:
              ...
              if zerosInSouper > zerosInLlvm:
                ...
                write_handle.write("LLVM is stronger\n")
                 ...
                 write_handle.write("Both are equal\n")
